    ...
    def show_progress(self):
        ...
        if self.n_steps > 0 and sim_config.disp_level > 0:
            if self.conv_fun is not None:
                print(f"  | CTM step {self.n_steps} conv: {self.diffs[-1]:.3e} time: {self.last_ctm_time} obj: {self.convergence:.6f}")
            else:
                print(f"  | CTM step {self.n_steps} conv: {self.diffs[-1]:.3e} time: {self.last_ctm_time}")
            ...
            for j in range(A.size[1]):
                with cur_loc(i,j):
                    if not ts.C1.is_changed(0,0):
                        ts.update(('C1', 'C4', 'T4'), ([0,0],[0,0],[0,0]), renorm_left(ts, Pl, Plb))
    return ts, sl
            ...
            for j in range(A.size[1]):
                with cur_loc(i,j):
                    if not ts.C2.is_changed(1,0):
                        ts.update(('C2', 'C3', 'T2'), ([1,0],[1,0],[1,0]), renorm_right(ts, Pr, Prb))
    return ts
            ...
            for i in range(A.size[0]):
                with cur_loc(i,j):
                    if not ts.C1.is_changed(-1,0):
                        ts.update(('C1', 'C2', 'T1'), ([-1,0],[2,0],[0,0]), renorm_top(ts, Pt, Ptb))
    return ts
            ...
            for i in range(A.size[0]):
                with cur_loc(i,j):
                    if not ts.C3.is_changed(2,1):
                        ts.update(('C3', 'C4', 'T3'), ([2,1],[-1,1],[0,1]), renorm_bottom(ts, Pb, Pbb))
    return ts
...
def get_projectors_left(ts: CTMTensors, chi: int
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """ 
    Returns the left projectors
    """
    tensors = (ts.C1[-1,-1],ts.C4[-1,2],ts.T1[0,-1],ts.T3[0,2],ts.T4[-1,0],ts.T4[-1,1],
            ts.A[0,0], ts.Ad[0,0], ts.A[0,1], ts.Ad[0,1])
...
def get_projectors_right(ts: CTMTensors, chi: int
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """ 
    Returns the right projectors
    """
    tensors = (ts.C2[2,-1],ts.C3[2,2],ts.T1[1,-1],ts.T2[2,0],ts.T2[2,1],ts.T3[1,2],
            ts.A[1,0], ts.Ad[1,0], ts.A[1,1], ts.Ad[1,1])
...
def get_projectors_top(ts: CTMTensors, chi: int
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """ 
    Returns the top projectors
    """
    tensors = (ts.C1[-1,-1],ts.C2[2,-1],ts.T1[0,-1],ts.T1[1,-1],ts.T2[2,0],ts.T4[-1,0],
            ts.A[0,0], ts.Ad[0,0], ts.A[1,0], ts.Ad[1,0])
...
def get_projectors_bottom(ts: CTMTensors, chi: int
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """ 
    Returns the bottom projectors
    """
    tensors = (ts.C3[2,2],ts.C4[-1,2],ts.T2[2,1],ts.T3[0,2],ts.T3[1,2],ts.T4[-1,1],
            ts.A[0,1], ts.Ad[0,1], ts.A[1,1], ts.Ad[1,1])
...
def renorm_left(ts: CTMTensors, Pl: np.ndarray, Plb: np.ndarray
        ) -> Tuple[Nested, Nested, Nested]:
    """

    """
    new_T4 = ncon([Plb[0,0], ts.all_T4[-1,0], ts.all_A[0,0], ts.all_Ad[0,0], Pl[0,1]], 'doleft_T4', 
            normalize=True).shift(-sim_config.px)
...
def renorm_right(ts, Pr, Prb):
    new_T2 = ncon([Prb[0,0], ts.all_T2[2,0], ts.all_A[1,0], ts.all_Ad[1,0], Pr[0,1]], 'doright_T2', 
            normalize=True).shift(sim_config.px)
...
def renorm_top(ts, Pt, Ptb):
    new_T1 = ncon([Ptb[-1,0], ts.all_T1[0,-1], ts.all_A[0,0], ts.all_Ad[0,0], Pt[0,0]], 'dotop_T1', 
            normalize=True).shift(-sim_config.py)
...
def renorm_bottom(ts, Pb, Pbb):
    new_T3 = ncon([Pbb[-1,0], ts.all_T3[0,2], ts.all_A[0,1], ts.all_Ad[0,1], Pb[0,0]], 'dobottom_T3', 
            normalize=True).shift(sim_config.py)
