...
def _compute_one_site_exci_norm(ts):
    ...
    n_tensors = [
        ts.Cs[0][-1,-1], ts.Ts[0][0,-1], ts.Cs[1][1,-1], ts.Ts[1][1,0],
        ts.Cs[2][1,1], ts.Ts[2][0,1], ts.Cs[3][-1,1], ts.Ts[3][-1,0]
    ]
    B_tensors = [
        ts.B_Cs[0][-1,-1], ts.B_Ts[0][0,-1], ts.B_Cs[1][1,-1], ts.B_Ts[1][1,0],
        ts.B_Cs[2][1,1], ts.B_Ts[2][0,1], ts.B_Cs[3][-1,1], ts.B_Ts[3][-1,0]
    ]
    Bd_tensors = [
        ts.Bd_Cs[0][-1,-1], ts.Bd_Ts[0][0,-1], ts.Bd_Cs[1][1,-1], ts.Bd_Ts[1][1,0],
        ts.Bd_Cs[2][1,1], ts.Bd_Ts[2][0,1], ts.Bd_Cs[3][-1,1], ts.Bd_Ts[3][-1,0]
    ]

    # Compute the ground state one-site reduced density matrix
    n_dm = get_single_site_dm(*n_tensors)
    nrm0 = ncon((ts.A[0,0], ts.Ad[0,0], n_dm), ([1,2,3,4,5],[1,6,7,8,9],[2,3,4,5,6,7,8,9]))
    ...
    nrm_exci = (ncon((ts.B[0,0], ts.Bd[0,0], n_dm), ([1,2,3,4,5],[1,6,7,8,9],[2,3,4,5,6,7,8,9])) +
            ncon((ts.A[0,0], ts.Bd[0,0], B_dm), ([1,2,3,4,5],[1,6,7,8,9],[2,3,4,5,6,7,8,9]))) / nrm0
...
def get_orth_basis(tensors):
    ...
    for i in A.x_major():
        with cur_loc(i):
            if not nrms.is_changed(0,0):
                n_tensors = [
                    tensors.Cs[0][-1,-1], tensors.Ts[0][0,-1], tensors.Cs[1][1,-1], tensors.Ts[1][1,0],
                    tensors.Cs[2][1,1], tensors.Ts[2][0,1], tensors.Cs[3][-1,1], tensors.Ts[3][-1,0]
                ]
                # Compute the ground state one-site reduced density matrix
                n_dm = get_single_site_dm(*n_tensors)
                nrm0 = ncon((tensors.A[0,0], tensors.Ad[0,0], n_dm), ([1,2,3,4,5],[1,6,7,8,9],[2,3,4,5,6,7,8,9]))
                nrms[0,0] = nrm0
                env_0 = ncon((tensors.Ad[0,0], n_dm), ([-1,6,7,8,9],[-2,-3,-4,-5,6,7,8,9]))
    ...
    def _apply_ops_h(A, B, ops):
            for i in A.x_major():
                with cur_loc(i):
                    ...
                    B[0,0] = B[0,0] - ncon((A[0,0], op_l), ([-1,-2,-3,1,-5],[-4,1]))
            return B
    ...
    def _apply_ops_v(A, B, ops):
            for i in A.x_major():
                with cur_loc(i):
                    ...
                    B[0,0] = B[0,0] - ncon((A[0,0], op_d), ([-1,-2,-3,-4,1],[1,-5]))
            return B
...
def get_dms(ts, only_gs=False):
    ...
    v_tensors = [
        C1[-1,-1], C2[1,-1], C3[1,2], C4[-1,2],
        T1[0,-1], T2[1,0], T2[1,1], T3[0,2], T4[-1,0], T4[-1,1], 
        A[0,0], A[0,1], Ad[0,0], Ad[0,1]
    ]

    # Tensors that are part of the horizontal reduced density matrix
    h_tensors = [
        C1[-1,0], C2[2,0], C3[2,2], C4[-1,2], 
        T1[0,0], T1[1,0], T2[2,1], T3[0,2], T3[1,2], T4[-1,1], 
        A[0,1], A[1,1], Ad[0,1], Ad[1,1]
    ]
...
def _get_dm_h(C1, C2, C3, C4, T1l, T1r, T2, T3l, T3r, T4, Al, Ar, Adl, Adr):
    ...
    Cc3 = ncon([C3.shift(px), T2.shift(px), T3r.shift(px), Adr.shift(px)], 'dm_right_Cc3')
